...
public class MazeAgent : Agent
{
    ...
    [Observable]
    public int currentCellPositionR = 0;

    [Observable]
    public int currentCellPositionC = 0;

    [Observable]
    public int currentCellPositionValue = 0;
    ...
    public override void Initialize()
    {
        ...
        // currentCellPositionR = 0;
        // currentCellPositionC = 0;
        currentDirection = 0;
    }
    ...
    public override void Heuristic(in ActionBuffers actionsOut)
    {
        ...
        if (Input.GetKey(KeyCode.W) && rayOutputs[0].HitTagIndex != 0)
        {
            // move forward
            forwardAction = 1;
    ...
    public override void OnEpisodeBegin()
    {
        isFull = 10;
        gameArea.ResetArea();
        GameObject[] cellCollider = GameObject.FindGameObjectsWithTag("cell");
        foreach (GameObject cell in cellCollider)
        {
            cell.gameObject.GetComponent<CellProp>().resetVisited();
        }
    }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            // Try to eat the cheese
            this.currentCellPositionR = collision.gameObject.GetComponent<CellProp>().cellPositionRow;
            this.currentCellPositionValue = collision.gameObject.GetComponent<CellProp>().cellPositionColumn;

            this.currentCellPositionValue = (int)collision.gameObject.GetComponent<CellProp>().wallProp;
            collision.gameObject.GetComponent<CellProp>().changeVisitedCount();
            int cellVisited = collision.gameObject.GetComponent<CellProp>().visitedCount;
            if (cellVisited > 5) cellVisited = 5;
            AddReward(0.2f - (cellVisited * 0.1f));
    ...
    private void CheckEnd()
    {
        ...
        heart.transform.localScale = new Vector3(5f, 5f, 5f);
        Destroy(heart, 4);
