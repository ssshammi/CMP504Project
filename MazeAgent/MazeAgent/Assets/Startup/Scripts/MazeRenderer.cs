...
public class MazeRenderer : MonoBehaviour
{
    ...
    [SerializeField]
    private Transform cellPrefab = null;
    ...
    private void Draw(WallState[,] maze)
    {
        ...
        for (int i = 0; i < width; ++i)
        {
            for (int j = 0; j < height; ++j)
            {
                ...
                var cellCollider = Instantiate(cellPrefab, transform) as Transform;
                cellCollider.position = position + new Vector3(0, -1.9f, 0);
                cellCollider.localScale = new Vector3(size * 0.7f, 1.0f, size * 0.7f);
                cellCollider.GetComponent<CellProp>().wallProp = cell - (int)WallState.VISITED;

                cellCollider.GetComponent<CellProp>().cellPositionRow = i;
                cellCollider.GetComponent<CellProp>().cellPositionColumn = j;
